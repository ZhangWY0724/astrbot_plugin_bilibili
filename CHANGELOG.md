## 未发布
- 修复专栏渲染模块缺少 `Optional` 导入导致插件加载失败的问题。
- 重写默认卡片模板，使其接近 Bilibili 动态流主体布局，并补充动态发布时间；新安装默认使用插件卡片渲染，不再依赖原生网页截图。
- 单张长图按正文宽度等比展示完整内容，不再缩成窄条或按固定高度裁切。
- 专栏动态通过 Opus 详情接口补全有序正文，在新版卡片模板中按原顺序展示全部文字和图片；详情失败时回退摘要与封面，超宽正文图通过 Bilibili CDN 缩至两倍展示宽度以控制内存。
- 增加定时动态检测、接口请求、新动态识别、过滤原因和推送结果日志，并避免在新增日志中输出完整订阅会话标识。
- 插件唯一标识调整为 `astrbot_plugin_bilibili_aikaid`，避免与上游插件冲突，并兼容迁移旧标识下的登录凭据和订阅数据。
- 渲染模式收敛为纯文本和插件模板卡片；移除 Browserless、Playwright 及原生网页截图链路，旧版 `native` 配置自动兼容为 `card`。
